...
let getTypeList idList env =
  List.map idList (fun id -> getTypeOfVar_exn id env)
...
let getTypeField objType fn prog = match objType with
  | ObjectType(cn) -> (let fieldList = getFieldList objType prog in
                       try let (fnf,ftf) = List.find_exn fieldList ~f:(fun (x,_) -> x = fn) in Some(ftf)  with Not_found -> None)
  | _ -> None
...
let getMethods obj prog = match prog with
  | Program classList -> (match obj with
      | ObjectType "Object" -> []
      | ObjectType cn -> (try let Class(n,pn,_,methods) = List.find_exn classList
                                  ~f:(function
                                      |Class(c,_,_,_) -> if cn = c then true else false
                                    )  in methods with Not_found -> (raiseRuntimeError ((stringOfType obj) ^ " is not defined inside program.")))
      | _ -> [])
...
let rec getMethodDefinition obj mn prog = match obj with
  | ObjectType cn -> (if cn = "Object" then None else let methods = (getMethods obj prog) in
                        try let methodDecl = (List.find_exn methods
                                                ~f:(function
                                                    |Method(_,n,_,_) -> if n = mn then true else false
                                                  )) in Some(methodDecl) with Not_found -> (getMethodDefinition (getParent obj prog) mn prog))
  | _ -> None
...
let rec findFirstIntersection list1 list2 = try let first = (List.find_exn list1 ~f:(fun x-> List.exists list2 (fun y-> y=x) ) ) in Some(first) with Not_found -> None
...
let rec isSubtype t1 t2 prog= match t1, t2 with
  ...
  | ObjectType(cn1), ObjectType(cn2) -> if cn1 = cn2 then true else let p = (getParent t1 prog) in if t2 = p then true else (isSubtype p t2 prog)
...
let eachElementOnce_exn l = List.iteri l ~f:(fun i x-> let xList = List.filter l ~f:(fun y -> x = y) in
                                              if List.length xList = 1 then () else raise (DuplicateElement i))

let methodsOnce_exn = function Class(_,_,_,methods) -> let methodsNames = List.map methods ~f:(function
    |Method(_,n,_,_) -> n)  in  try eachElementOnce_exn methodsNames with DuplicateElement index -> raise (DuplicateMethod (List.nth_exn methods index) )

let fieldsOnce_exn = function Class(_,_,fields,_) -> let fieldsNames = List.map fields ~f:(function
    |(n,_) -> n)  in  try eachElementOnce_exn fieldsNames with DuplicateElement index -> raise (DuplicateField (List.nth_exn fields index) )

let methodName = function Method(_,n,_,_) -> n
let goodOverride m1 m2 prog = match m1,m2 with
  |Method(t1,_,args1,e1), Method(t2,_,args2,e2) -> if args1 = args2 && e1 = e2 && isSubtype t1 t2 prog then true else false
...
let goodInheritance cl prog = let Class(cn,pn,_,methods) = cl in
  ...
  List.iter methods ~f:(fun m ->
      List.iter parentMethods ~f:(fun mp ->
          if methodName m = methodName mp then if goodOverride m mp prog then () else raise(BadMethodOverriding m) else ()) )
